...
EvalvidClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EvalvidClient")
    ...
    .AddAttribute ("RemoteAddress",
                   ...
                   AddressValue (),
                   MakeAddressAccessor (&EvalvidClient::m_peerAddress),
                   MakeAddressChecker ())
    ...
    .AddTraceSource ("TxWithAddresses", "A new packet is created and is sent",
                     MakeTraceSourceAccessor (&EvalvidClient::m_txTraceWithAddresses),
                     "ns3::Packet::TwoAddressTracedCallback")
    .AddTraceSource ("RxWithAddresses", "A packet has been received",
                     MakeTraceSourceAccessor (&EvalvidClient::m_rxTraceWithAddresses),
                     "ns3::Packet::TwoAddressTracedCallback")
    ;
  return tid;
}
...
void
EvalvidClient::SetRemote (Address ip, uint16_t port)
{
...
EvalvidClient::StartApplication (void)
{
  ...
  if (receiverDumpFile.fail())
    {
      NS_FATAL_ERROR(">> EvalvidClient: Eess &lorror while opening output file: " << receiverDumpFileName.c_str());
      return;
    }
...
EvalvidClient::Send (void)
{
  ...
  seqTs.SetSeq (0);
  p->AddHeader (seqTs);
//std::cout<<Simulator::Now().GetSeconds()<<" client "<<localAddress<<std::endl; 
