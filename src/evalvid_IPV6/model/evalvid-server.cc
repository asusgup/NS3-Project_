...
EvalvidServer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EvalvidServer")
    ...
    .AddTraceSource ("TxWithAddresses", "A new packet is created and is sent",
                     MakeTraceSourceAccessor (&EvalvidServer::m_txTraceWithAddresses),
                     "ns3::Packet::TwoAddressTracedCallback")
    .AddTraceSource ("RxWithAddresses", "A packet has been received",
                     MakeTraceSourceAccessor (&EvalvidServer::m_rxTraceWithAddresses),
                     "ns3::Packet::TwoAddressTracedCallback")
    ;
  return tid;
}
   ...
   if (socket == 0)
    {
      ...
      InetSocketAddress local = InetSocketAddress (Ipv4Address::GetAny (),
                                                   m_port);
      socket->Bind (local);
      std::cout<<"Address is ipv4"<<std::endl; 
  ...
  if (socket6 == 0)
    {
      ...
      Inet6SocketAddress local = Inet6SocketAddress (Ipv6Address::GetAny (),
                                                   m_port);
      socket6->Bind (local);
      std::cout<<"Address is ipv6"<<std::endl;
...
EvalvidServer::Send ()
{
  std::cout<<Simulator::Now().GetSeconds()<<" "<<m_peerAddress<<std::endl;
      ...
      for(int i=0; i<m_videoInfoMapIt->second->numOfUdpPackets - 1; i++)
        {
          ...
          p->AddHeader (seqTs);
         // Address x;
          //m_txTraceWithAddresses (p, x, Inet6SocketAddress (Ipv6Address::ConvertFrom (m_peerAddress),8000));
