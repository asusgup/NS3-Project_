...
int main (int argc, char *argv[])
{
  ...
  Ipv4Address remoteHostAddr = internetIpIfaces.GetAddress (1,0);
 ...
 // echoClient.SetAttribute ("MaxPackets", UintegerValue (20));
  ...
  Ptr<OutputStreamWrapper> appStream = ascii.CreateFileStream ("AppPacketTrace.tr");
  *appStream->GetStream () << "time(s)\ttx/rx\tNodeID\tIMSI\tPktsize(bytes)\tIP[src]\tIP[dst]" << std::endl;
  ...
  Ipv4Address LocalAddrs =  clientApps.Get (0)->GetNode ()->GetObject<Ipv4L3Protocol> ()->GetAddress (1,0).GetLocal ();
  ...
  localAddrs = Ipv4Address::GetZero ();
  //std::cout<<localAddrs<<std::endl;
  //Downward: Tx by Remote Host / Rx by UE
