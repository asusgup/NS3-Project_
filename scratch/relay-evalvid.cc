...
int main (int argc, char *argv[])
{
  ...
  EvalvidServerHelper echoServer(echoPort);
  echoServer.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
  echoServer.SetAttribute ("SenderDumpFilename", StringValue("sd_a01_lte"));
  echoServer.SetAttribute ("PacketPayload",UintegerValue(1014));
  
  ApplicationContainer serverApps = echoServer.Install(remoteHost);
    serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (4.0));
  
  echoServer.SetAttribute ("SenderDumpFilename", StringValue("sd_a01_lte1"));
  ApplicationContainer serverApps1 = echoServer.Install(remoteHost);
  serverApps1.Start(Seconds(6.0));
  serverApps.Stop (Seconds (10.0));


  //Set echo client in the remote UEs
  EvalvidClientHelper echoClient(remoteHostAddr,echoPort);
  echoClient.SetAttribute ("ReceiverDumpFilename", StringValue("rd_a01_lte"));
 // echoClient.SetAttribute ("MaxPackets", UintegerValue (20));
  ...
  echoClient.SetAttribute ("ReceiverDumpFilename", StringValue("rd_a01_lte1"));
  ApplicationContainer clientApps1 = echoClient.Install (ueNodes.Get(1));
  clientApps1.Start (Seconds (6.0));
  clientApps1.Stop (Seconds (10.0));

 // Simulator::Schedule (Seconds (5.0),&(ApplicationContainer::Start),&clientApps,Seconds(6.0));
  //Simulator::Schedule (Seconds (5.0),&(ApplicationContainer::Stop),&clientApps,Seconds(12.0));
...
/*  AsciiTraceHelper ascii;
  ...
  *appStream->GetStream () << "time(s)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]" << std::endl;
  ...
  Ipv6Address localAddrs1 = clientApps.Get (0)->GetNode ()->GetObject<Ipv6L3Protocol> ()->GetAddress (1,1).GetAddress ();
  std::cout<<localAddrs<<std::endl;
  std::cout<<localAddrs1<<std::endl;
...
*/
  ...
  AnimationInterface anim ("animater_final.xml");
  anim.SetConstantPosition (ueNodes.Get(0), 10.0, 0.0);
  anim.SetConstantPosition (ueNodes.Get(1), 20.0, 0.0);
  anim.SetConstantPosition (enbNode.Get(0), 0.0, 10.0);
  anim.SetConstantPosition (remoteHostContainer.Get(0), 0.0, 15.0);
