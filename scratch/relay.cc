...
int main (int argc, char *argv[])
{
  ...
  Address localAddress=Inet6SocketAddress (Ipv6Address::GetAny (), 8000);
  //Set echo server in the remote host
  OnOffHelper sidelinkClient ("ns3::UdpSocketFactory", remoteHostAddr);
  sidelinkClient.SetConstantRate (DataRate ("16kb/s"), 200);

  ApplicationContainer clientApps = sidelinkClient.Install (remoteHost);
  ...
  for (uint16_t ac = 0; ac < clientApps.GetN (); ac++)
        {
          ...
          clientApps.Get (ac)->TraceConnect ("TxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, appStream, localAddrs));
          oss.str ("");
        }
      ...
      for (uint16_t ac = 0; ac < serverApps.GetN (); ac++)
        {
          ...
          serverApps.Get (ac)->TraceConnect ("RxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, appStream, localAddrs));
          oss.str ("");
        }
